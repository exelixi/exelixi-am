...
import xyz.exelixi.interp.Channel.InputEnd;
import xyz.exelixi.interp.Channel.OutputEnd;
	...
	private void instantiateNodes(Channel[][] internalSourcePortChannel, Channel[][] intrnalSinkPortChannel){
		...
		for(int nodeIndex=0; nodeIndex<nbrNodes; nodeIndex++){
			...
			if(node.getContent() instanceof ActorMachine){
				...
				throw new UnsupportedOperationException("Network node must be instances of ActorMachine or Network in the simulator. " + node.getName() + " is instance of class "+ node.getContent().getClass().getCanonicalName());
			}
		}
	}
