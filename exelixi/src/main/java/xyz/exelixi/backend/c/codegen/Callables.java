...
public interface Callables {
    ...
    default void generateSourceCode() {
        ...
        declareHeaderCallables();

        declarePrintCallables();

        declareCallables();

        declareEnvironmentForCallablesInScope(task);

        defineCallables();
    }

    default void generateHeaderCode() {
        decalreHeaderCallables();
    }


    default void declarePrintCallables() {
        backend().emitter().emit("// PRINT FUNCTIONS");
        backend().emitter().emit("void println(char *text) {");
        backend().emitter().increaseIndentation();
        backend().emitter().emit("puts(text);");
        backend().emitter().decreaseIndentation();
        backend().emitter().emit("}");

        backend().emitter().emit("void print(char *text) {");
        backend().emitter().increaseIndentation();
        backend().emitter().emit("fputs(text, stdout);");
        backend().emitter().decreaseIndentation();
        backend().emitter().emit("}");
        backend().emitter().emit("");
    }

    default void declareHeaderCallables() {
        preprocessor().preprocessor_user_include("callables");
        backend().emitter().emit("");
    }

    default void declarePrintCallablesPrototype() {
        backend().emitter().emit("// PRINT PROTOTYPES");
        backend().emitter().emit("void println(char *text);");
        backend().emitter().emit("void print(char *text);");
    }


    default void decalreHeaderCallables() {
        preprocessor().preprocessor_ifndef("callables");
        preprocessor().preprocessor_define("callables");
    ...
    default void externalCallableDefinitionPrototype(IRNode varDecl) {
    }

    default void externalCallableDefinitionPrototype(VarDecl varDecl) {
        if (varDecl.isExternal()) {
            Type type = backend().types().declaredType(varDecl);
            assert type instanceof CallableType : "External declaration must be function or procedure";
            CallableType callable = (CallableType) type;
